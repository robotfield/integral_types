...
struct always_integral<Int_t, T, Ts...> {
  static constexpr bool value = std::is_integral_v<Int_t<bit_sizeof_v<T>>>
...
struct always_integral_count {
  static constexpr bool value = std::is_integral_v<Int_t<bits>>
                             && always_integral_count<Int_t, bits - 1>::value;
};
template <template <size_t> class Int_t>
struct always_integral_count<Int_t, 0> {
  static constexpr bool value = std::is_integral_v<Int_t<0>>;
};
...
struct same_size_invariant<Int_t, T, Ts...> {
  static constexpr bool value = sizeof(Int_t<bit_sizeof_v<T>>) == sizeof(T)
                             && same_size_invariant<Int_t, Ts...>::value;
};

static_assert(
  bit_sizeof_v<char> == CHAR_BIT,
  "bit_sizeof failed cxx_std_20 compilation check");
static_assert(
  bit_sizeof_v<size_t> == sizeof(size_t) * CHAR_BIT,
  "bit_sizeof failed cxx_std_20 compilation check");
...
static_assert(
  ...
  "int_t failed cxx_std_20 compilation check -- not an integral type");
static_assert(
  always_integral<
    uint_t,
    unsigned char,
  ...
  "uint_t failed cxx_std_20 compilation check -- not an integral type");

static_assert(
  always_integral_count<int_least_t, bit_sizeof_v<size_t>>::value,
  "int_least_t failed cxx_std_20 compilation check -- not an integral type");
static_assert(
  always_integral_count<uint_least_t, bit_sizeof_v<size_t>>::value,
  "uint_least_t failed cxx_std_20 compilation check -- not an integral type");
...
static_assert(
  same_size_invariant<
    ...
    signed long,
    signed long long>::value,
  "int_t failed cxx_std_20 compilation check -- bit size of available integral "
...
static_assert(
  same_size_invariant<
    ...
    unsigned long,
    unsigned long long>::value,
  "uint_t failed cxx_std_20 compilation check -- bit size of available "
...
static_assert(
  same_size_invariant<
    ...
    signed long,
    signed long long>::value,
  "int_least_t failed cxx_std_20 compilation check -- bit size of available "
  "integral type does not yield a type of the same bit size");
static_assert(
  same_size_invariant<
    uint_least_t,
    ...
    unsigned long,
    unsigned long long>::value,
  "uint_least_t failed cxx_std_20 compilation check -- bit size of available "
  "integral type does not yield a type of the same bit size");

static_assert(
  sizeof(int_fit_t) <= sizeof(size_t) || sizeof(uint_fit_t) <= sizeof(size_t),
  "int_fit_t/uint_fit_t failed cxx_std_20 compilation check -- larger than "
  "size_t");
